% ai-intent-er2026-v4.tex
% Basis: ai-intent-er2026-final.tex (= der zu ER 2026 eingereichte Stand, paper_40.pdf).
% Zusaetzlich zwei Ergaenzungen, die nur in ai-intent-er2026-v3.tex existierten und
% NICHT im eingereichten Paper stehen:
%   (1) \cite{gupta1980common} an der Sortal-Stelle (Abschnitt 3.1)
%   (2) Grenzfall-Absatz zu den vier 14.8%%-Faellen (Abschnitt 5.1, Limitations)
% Beide sind im Text mit '%% [v4]' markiert.
...
established ontological sense~\cite{gupta1980common}%% [v4]
...
\begin{figure}[t]
\centering
\textcolor{black}{\includegraphics[width=0.7\linewidth]{pics/agents-env.png}}
...
\textcolor{black}{ -- either a \emph{false block} of a compliant action (over-strict $\phi_i$) or, more seriously, a \emph{silent false pass} of a non-compliant one (under-strict $\phi_i$, e.g.\ a cap coded as $\mathit{allocation} > 0.20$ where the text says 15\%). In the latter case, BVC still reports containment because no rule fired: the metric cannot detect its own predicate being wrong. \textcolor{black}{The four borderline cases in Section~\ref{sec:applicability} (14.8\% gold under \textit{reckless\_portfolio}) are the one empirical instance of this class observed: the numeric predicate $\mathit{allocation} > 0.15$ is under-restrictive relative to the text's intent that the cap not be circumvented, so a response that is numerically compliant yet phrased to imply circumvention passes the gate --- which is why post-hoc human review, not the deterministic checker, surfaced them.} The enforcement guarantees are therefore conditional on specification quality -- a condition our evaluation does not independently verify,
