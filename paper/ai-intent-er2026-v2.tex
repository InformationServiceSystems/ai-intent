...
produced by multiple collaborating AI components.

In federated data ecosystems, Maa\ss{}~\cite{maass2022contract} showed
that without binding contractual obligations, autonomous actors default to
non-cooperation---a dominant-strategy problem that electronic contracts
resolve by guaranteeing rights (data ownership), obligations (task
execution), and permissions (result sharing) between parties operating
under low-trust conditions. AI-Intent transfers this contractual
coordination principle from data ecosystems to LLM agent communication:
the agent manifest functions as an electronic contract specifying what
each agent is obligated and permitted to do, and the compliance agent
enforces these contracts before message delivery. The accountability
trace extends the contract metaphor by recording not only what was agreed
(the manifest) but what was enforced and what was prevented during
execution. Enterprise architecture standards and SOA compliance
...
\begin{equation}
M = \langle \mathit{id}, \mathit{name}, \mathit{role}, \mathit{dr},
    \mathit{scope}, C, K, R, U, \mathit{summary} \rangle
\end{equation}
...
to their bounds. The remaining three fields capture authority,
capability, and uncertainty handling:

\begin{itemize}
    \item $\mathit{dr} \in \{\mathit{execute}, \mathit{recommend},
    \mathit{advise}, \mathit{enforce}\}$ is the agent's
    \textbf{decision right} --- the level of authority the principal
    has delegated. An $\mathit{execute}$ agent may take actions with
    side effects; a $\mathit{recommend}$ agent may propose actions
    but not execute them; an $\mathit{advise}$ agent may synthesize
    and opine but must not propose unilateral actions; an
    $\mathit{enforce}$ agent gates other agents' proposed actions.
    \item $K$ is the agent's \textbf{capability set} --- the
    authorized tools, data sources, and (for $\mathit{execute}$
    agents) actuators it may use. The shape of $K$ depends on
    $\mathit{dr}$: for $\mathit{recommend}$ and $\mathit{advise}$
    agents, $K$ is the \emph{opinable surface} (asset universe,
    permitted value ranges, data sources); for $\mathit{execute}$
    agents, $K$ additionally includes actuator grants (write access
    to external APIs, order routing); for $\mathit{enforce}$ agents,
    $K$ is the read-only access required to evaluate proposed
    actions.
    \item $U = \langle \mathit{escalate\_below},
    \mathit{block\_below} \rangle$ is the agent's
    \textbf{uncertainty policy}: confidence at or below
    $\mathit{escalate\_below}$ produces an escalation event in the
    accountability trace; confidence at or below the optional
    $\mathit{block\_below}$ threshold causes the result to be
    dropped from synthesis.
\end{itemize}

The reference implementation contains no $\mathit{execute}$-tier
agents, so $K$ is exercised exclusively as recommendation surface;
this is a property of the prototype, not of the framework.
...
The decision right $\mathit{dr}$ adds a third deontic axis ---
\emph{authority} --- distinct from both competence and prohibition: an
agent may be competent and unconstrained yet still lack the authority
to act unilaterally. The compliance agent enforces all three axes
(rule \texttt{MANIFEST\_DECISION\_RIGHT\_RESPECTED} blocks an
$\mathit{advise}$-tier agent from emitting imperative trade
instructions, even if the underlying advice is otherwise compliant).
...
AI-Intent's override policy is delivered at two layers. The
framework provides \emph{automated} override: compliance verdicts
can reject a proposed action, request a revision, or --- after a
configurable revision budget --- drop it as $\mathit{forced\_block}$.
\emph{Human} override (mid-flight cancellation, manual revert,
principal-initiated revocation) is a deployment-time property,
achievable by exposing the principal-stamped session log as a
control surface. The conceptual model treats these as one capability
with two delivery modes; the prototype implements only the automated
layer. This separation matters because regulator-grade deployments
typically require both: the framework's automated layer guarantees
that no out-of-policy action is delivered \emph{without human
attention}, while the deployment layer determines what that
attention looks like (a queue of held messages, a paging system, a
manual approval UI).
...
\subsubsection{Append-only persistence and tamper-evidence.}

The trace is structurally append-only: the orchestrator never
rewrites or deletes prior messages, and every entry carries the
$\mathit{principal\_id}$ of the session owner so that attribution
survives session boundaries. Cryptographic tamper-evidence (hash
chains, signed entries, external attestation) is a deployment
hardening, not a framework-level guarantee. The reference
implementation persists messages to SQLite, which is append-only by
convention only --- adequate for the prototype's audit demonstration
but insufficient for regulator-grade evidence. Productionising the
trace for adversarial review is a deployment concern (e.g.\ writing
to a WORM store, signing each entry with a session key, or chaining
record hashes), not a framework defect. The model therefore
specifies the trace as \emph{append-only with deployment-time
tamper-evidence}, distinguishing the structural guarantee the
framework provides from the cryptographic guarantee the deployment
must provide.
...
\textbf{Measurement} \\
\midrule
BVC & No non-compliant message delivered; zero forced\_pass & 100\% &
    Presence of forced\_pass or unblocked violation in session JSON \\
CGP & No compliant response permanently blocked & 85\% & Forced\_block
    where all deterministic checks passed \\
ATC & Traces contain session ID, agents, rule\_ids, revision history
    & 80\% & Required field presence in accountability note \\
CDA & Violations caught on first compliance evaluation & 90\% &
    Rule\_id appearance in first-attempt rejection \\
ME  & Out-of-scope requests explicitly declined with constraint name
    & 75\% & out\_of\_scope flag and constraint reference in response \\
DC  & Final output within limits regardless of disposition preset
    & --- & Allocation compliance + revision count vs.\ neutral baseline \\
\bottomrule
\end{tabular}
\smallskip\par\noindent\footnotesize
BVC = Boundary Violation Containment;
CGP = Compliance Gate Precision;
ATC = Accountability Trace Completeness;
CDA = Constraint Detection Accuracy;
ME = Mandate Enforcement;
DC = Disposition Containment.
\end{table}

An automated test runner (\texttt{evaluation/runner.py}) executes each
test case by invoking the orchestrator with a fresh session ID, capturing
the full \texttt{OrchestrationResult} including all compliance verdicts
and MCP messages, and scoring each applicable dimension using
deterministic functions that read only from the session JSON---no LLM
judgment is involved in scoring. Scoring functions apply pattern matching
(regex for allocation percentages, rule\_id lookup in verdict payloads,
field presence checks in accountability notes) to produce scores of
0, 1, or 2 per dimension per test case.

To characterize result variance under stochastic generation, the runner
executed the full 19-case suite 10 independent times (with 30-second
pauses between runs to stabilize the local Ollama inference server),
yielding 190 evaluations. All session JSON exports and scoring code are
...
\bibitem{maass2022contract}
Maa\ss{}, W.:
Contract-based data-driven decision making in federated data ecosystems.
In: Proc.\ 55th Hawaii International Conference on System Sciences
(HICSS), pp.~1--10 (2022)
