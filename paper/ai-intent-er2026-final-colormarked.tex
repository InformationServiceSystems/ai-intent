...
%\medskip
To address these gaps, we propose \textsc{AI-Intent}, with one pillar per gap. \textbf{\textit{Declaration}} introduces the \textit{Mandate}, a first-class artifact encoding an agent's binary action boundary, decision rights, capabilities, and risk policies. \textbf{\textit{Enforcement}} introduces \textit{Intent Enforcement}, a mandatory gatekeeper that evaluates every \textit{Proposed Action} against the applicable \textit{Mandate} and produces a \textit{Compliance Verdict} before delivery. \textbf{\textit{Auditability}} derives an \textit{Accountability Trace} from the \textit{Log Entry} of every verdict. \textcolor{blue}{It is important to note that \textsc{AI-Intent} is not a new modeling language, nor a syntactic extension of an existing one; it is a domain-independent \emph{conceptual model}, comprising governance constructs, their metamodel relationships, and well-formedness constraints.
...
\textcolor{blue}{We scope our review to work proposing modeling constructs for agent governance (Section~\ref{sec:aomodeling}); multi-agent implementation frameworks (Section~\ref{sec:mala}) and AI-governance policy work (Section~\ref{sec:aigov}) serve as deployment substrate and normative context.}

\subsection{Agent-Oriented Conceptual Modeling}
\label{sec:aomodeling}

\textcolor{blue}{Four influential modeling approaches are examined through the lens of the three gaps: the goal-oriented $i^*$~\cite{yu2011social} and Tropos~\cite{bresciani2004tropos}, and the more strictly agent-oriented GAIA~\cite{wooldridge2000gaia} and NMAS~\cite{boella2006introduction}. We group them because each supplies constructs that a governance model must either reuse or replace.}
...
\textbf{Normative Multi-Agent Systems~(NMAS)}~\cite{governatori2005meeting,boella2006introduction,dignum1999autonomous} address obligation and prohibition directly~\cite{vonwright1951essay}, and defeasible deontic logic~\cite{governatori2005meeting} further allows norms to have exceptions and
...
%\textcolor{blue}{The recent trustworthy-agent frameworks supply runtime auditing and provenance but no declarative modeling constructs for scope, prohibition, or risk. Only AI-Intent covers all eight capabilities, which is the gap the framework is designed to close.}
...
\textcolor{blue}{The analysis of the literature in Section 2 leaves specific gaps: no modeling framework combines declared scope boundaries, enforceable prohibitions, and audit-native communication, and the recent trustworthy-agent line supplies runtime mechanisms without the modeling constructs beneath them. \textsc{AI-Intent} closes these as a metamodel of governance constructs, their relationships, and well-formedness conditions, independent of any implementation. Section~\ref{sec:ontology} states its ontological commitments; Section~\ref{sec:pillars} presents the constructs, distinguishing UFO-derived constructs from design choices layered on top.}
...
standard deontic
semantics~\cite{vonwright1951essay,governatori2005meeting} as follows. %\textcolor{blue}{We note, however, that the dependence on UFO is for ontological categorization of \textsc{AI-Intent}'s constructs, not for its runtime operation: a deployment operationalizes these categories without ontological inference.}
...
\textcolor{blue}{\textbf{Intent scope as a sortal boundary.} We use \emph{sortal} in its
...
\toprule
\textcolor{blue}{\textbf{Construct}} & \textcolor{blue}{\textbf{UFO category / OntoUML stereotype}} &
\textcolor{blue}{\textbf{Src}} & \textcolor{blue}{\textbf{Realization in reference impl.}} \\
\midrule
\textcolor{blue}{Principal} & \textcolor{blue}{Intentional object, \uml{kind}} & \textcolor{blue}{des} &
    \textcolor{blue}{Human/institutional owner (config)} \\
\textcolor{blue}{Agent} & \textcolor{blue}{Intentional object w/ dispositions, \uml{kind}} & \textcolor{blue}{der} &
    \textcolor{blue}{Orchestrator $+$ three sub-agents} \\
\textcolor{blue}{Mandate} & \textcolor{blue}{Normative description (\emph{not} a goal)} & \textcolor{blue}{des} &
    \textcolor{blue}{Pydantic schema injected into input spec} \\
\textcolor{blue}{Intent scope} & \textcolor{blue}{Sortal universal} & \textcolor{blue}{der} &
    \textcolor{blue}{Natural-language scope string} \\
\textcolor{blue}{Boundary constraint} & \textcolor{blue}{Negative duty, \uml{mode}} & \textcolor{blue}{des} &
    \textcolor{blue}{Predicate in the rule registry} \\
\textcolor{blue}{Proposed Action} & \textcolor{blue}{Event (non-repeatable), \uml{event}} & \textcolor{blue}{der} &
    \textcolor{blue}{MCP message, pre-delivery} \\
\textcolor{blue}{Compliance Verdict} & \textcolor{blue}{Mode dependent on the Proposed Action} & \textcolor{blue}{der} &
    \textcolor{blue}{Verdict payload with constraint IDs} \\
\textcolor{blue}{Log Entry} & \textcolor{blue}{Event record, \uml{event}} & \textcolor{blue}{der} &
    \textcolor{blue}{SQLite row} \\
\textcolor{blue}{Accountability Trace} & \textcolor{blue}{Situation from session events, \uml{situation}} &
    \textcolor{blue}{der} & \textcolor{blue}{Session JSON export} \\
\textcolor{blue}{Regulatory Framework} & \textcolor{blue}{External normative description} & \textcolor{blue}{des} &
    \textcolor{blue}{Rule registry (MiFID~II)} \\
\bottomrule
\end{tabularx}
\end{table}
\end{comment}
...
\multirow[t]{4}{*}{Declaration}
  & Principal            & Intentional object, \uml{kind}                                & des \\
  & Agent                & Intentional object w/ dispositions, \uml{kind}                & der \\
  & Mandate              & Normative description, \uml{relator} & des \\
  & Proposed Action      & Event (non-repeatable), \uml{event}                           & der \\
\midrule
\multirow[t]{3}{*}{Enforcement}
  & Regulatory Framework & External normative description, \uml{category}                & des \\
  & Intent Enforcement   & Role played by the Compliance Agent, \uml{role}        & des \\
  & Compliance Verdict   & Relator mediating Proposed Action and Mandate, \uml{relator}  & des \\
\midrule
\multirow[t]{2}{*}{Auditability}
...
\textcolor{blue}{We chose investment advisory for its regulatory grounding under MiFID~II: constraints are imposed by regulation, not authored by us.} The reference implementation comprises four agents in a hierarchical architecture:
%(see Figure~\ref{fig:architecture}): 
a \textbf{\textit{Central Orchestrator}} that coordinates task delegation and synthesis, and three specialist sub-agents, \textbf{\textit{Stocks}}, \textbf{\textit{Bonds}}, and \textbf{\textit{Materials}}, each governed by a dedicated Mandate. The \textbf{\textit{Compliance Agent}} intercepts all communication between the Orchestrator and the sub-agents, producing a Compliance Verdict for every Proposed Action before delivery. \textcolor{blue}{No sub-agent is reachable except through the Compliance Agent, realizing requirement \textit{R1 (non-bypassability)}. \textit{Independence (R2)} follows from the Compliance Agent running as a distinct process from the agents it evaluates. Verdict decidability (R3) is achieved by a deterministic predicate checker over structured agent output, backed by a narrow semantic fallback -- a deliberately minimal choice, sufficient to show the construct is operable rather than the most capable checker the framework admits. \textit{Revision terminates (R4)} in a bounded loop that ends in delivery or a recorded forced block.
...
Table~\ref{tab:mandates} summarizes the Mandate of each agent, with constraints annotated by their deontic type: \textbf{F} (prohibition) or \textbf{O} (obligation).~\footnote{\label{fn:materials}The complete source code of the reference implementation is available in the online repository at \url{https://anonymous.4open.science/r/ai-intent-09F2}.}
...
\begin{figure}[t]
\centering
\textcolor{blue}{\includegraphics[width=0.7\linewidth]{pics/agents-env.png}}
...
\textbf{\textit{ATC (71.3\%}}, \textit{below 80}\%\textbf{\textit{).}} Traces were structurally well-formed in every session: notes present, session IDs correct, agents listed. However, Llama~3.1~8B omitted violated rule identifiers in 28.7\% of sessions. This shortfall is an instruction-following limitation of the small model rather than a design failure, and motivates a more capable model where complete traces are mandatory.  
...
Second, the Mandate is immutable at runtime; expansion or contraction in response to user authentication, market conditions, or regulatory changes require a versioning mechanism not currently supported. Third, unlike defeasible deontic logic~\cite{governatori2005meeting}, \textsc{AI-Intent} constraints have no exception structure -- a deliberate design choice for the regulated financial domain, where overrides are themselves regulated activities, but a limit in domains where exceptions are common.

\textit{Specification-predicate consistency.} Recall that each boundary constraint is a triple $c_i = \langle \mathit{text}_i, \phi_i, \tau_i \rangle$ (Section~\ref{sec:pillars}). If $\phi_i$ is miscoded relative to $\mathit{text}_i$, the Compliance Agent silently enforces the wrong rule and the Accountability Trace records that mismatch as if it were intended 
%\textcolor{blue}{This matters because the enforcement guarantees of Section~\ref{sec:applicability} are conditional on specification quality: a correct gate enforcing a wrong predicate yields 
\textcolor{blue}{ -- either a \emph{false block} of a compliant action (over-strict $\phi_i$) or, more seriously, a \emph{silent false pass} of a non-compliant one (under-strict $\phi_i$, e.g.\ a cap coded as $\mathit{allocation} > 0.20$ where the text says 15\%). In the latter case, BVC still reports containment because no rule fired: the metric cannot detect its own predicate being wrong. The enforcement guarantees are therefore conditional on specification quality -- a condition our evaluation does not independently verify,
...
\textit{Model operativity depends on instruction following.} A Mandate is operative only to the extent that the deployed model follows its input specification -- the ATC shortfall discussed in Section~\ref{sec:applicability} demonstrates this. 
...
%\textcolor{blue}{\textbf{Scope of validity and conditions for transfer.}} 
\textbf{External validity.} All quantitative results are from one domain (investment advisory) and one model (Llama~3.1~8B).
%the 12.9\% self-compliance rate is specific to the materials agent's allocation constraint. 
\textcolor{blue}{We therefore position the results as a \emph{proof of concept}: they establish that the constructs are operable and that external enforcement is necessary in at least one regulated setting.
%, not that the framework generalizes without further study. 
The \emph{domain-independent} core (metamodel, pillars,  well-formedness conditions) is separable from \emph{domain-specific}  content (Mandate text, predicate registry, risk parameters), and is expected to transfer where (i)~competence admits a sortal in/out-of-scope boundary; (ii)~norms are prohibitions expressible as machine-evaluable predicates; (iii)~Mandates are session-stable; and  (iv)~the domain tolerates lexicographic constraint priority, so a compliant non-answer is acceptable.
%--- as in regulated finance, but not emergency triage. 
Transfer is poor where norms admit routine exceptions  (Section~\ref{sec:expressiveness}), resist decidable predicates, or require dynamic renegotiation. 
Replication in other regulated domains (e.g., clinical decision support) is the primary next step.}

\textbf{Construct validity.} 
%The six dimensions map to the framework's three claims. BVC and CGP directly measure the Compliance Agent's core; ATC and CDA are proxies to trace-population and detection latency, respectively.
